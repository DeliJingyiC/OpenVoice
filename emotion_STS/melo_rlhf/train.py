...
def initial_buffer_RL(
    ...
    with torch.no_grad():
        ...
        for batch_idx, (
            ...
            # print('emo_audio_path_list_real',emo_audio_path_list_real)
            ...
            # latent_sim = latent_sim.squeeze()
            # euclidean_distances = euclidean_distances.squeeze()
            print('euclidean_distances',euclidean_distances)
            print('latent_sim',latent_sim)
            emotion_loss=batchwise_emotion_regularizer(euclidean_distances,latent_sim)
            print('after emotion_loss_syn 879',emotion_loss)
            ...
            # emotion_reward_list_syn=emotion_reward_list_syn.to(rank)
            ...
            # latent_sim_syn = latent_sim_syn.squeeze()
            # euclidean_distances_syn = euclidean_distances_syn.squeeze()
            ...
            speaker_reward_list2 = 1-torch.cdist(emb1, emb2, p=2)
            speaker_reward_list2 = torch.tensor(speaker_reward_list2).to(rank)
...
def initial_buffer_RL_07222024(
    ...
    with torch.no_grad():
        ...
        # audio = torch.tensor(audio).float().to(rank) for path in input_audio_list_real]
            ...
            speaker_reward_list = [speaker_func(pcm.to(rank), pcm2.to(rank), hps.data.sampling_rate) for pcm, pcm2 in zip(pcm_list, pcm2_list)]
...
def train_and_evaluate(
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        speaker_reward_list = torch.tensor(data['speaker_reward_list']).cuda(rank, non_blocking=True).view(-1)
        emotion_reward_list = torch.tensor(data['emotion_reward_list']).cuda(rank, non_blocking=True).view(-1)
        ...
        speaker_reward_list_syn = torch.tensor(data['speaker_reward_list_syn']).cuda(rank, non_blocking=True).view(-1)
        emotion_reward_list_syn = torch.tensor(data['emotion_reward_list_syn']).cuda(rank, non_blocking=True).view(-1)
        ...
        # print('speaker_reward_list_syn',speaker_reward_list_syn)
        # print('emotion_reward_list_syn',emotion_reward_list_syn)
        # print('input_real_spec',input_real_spec)
        # print('input_real_spec_len,',input_real_spec_len)
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                #cosine
                # reward=0.1*emotion_reward_list-1*loss_gen_all
                #batchwise emotion regularizer
                reward =-1*emotion_loss-0.1*loss_gen_all
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                if net_dur_disc is not None:
                    loss_dur_gen, losses_dur_gen = generator_loss(y_dur_hat_g)
                    loss_gen_all += loss_dur_gen
                reward=0*speaker_reward_list_syn+1*emotion_reward_list_syn -1*loss_gen_all
                log_p_pre_syn = log_p_pre_syn.squeeze()
                total_loss=-(log_p_reward_yhat*reward + (0*KL_)) -0*log_p_pre_syn 
                total_loss_=loss_gen_all
                total_loss_reduced = total_loss_.mean(dim=0)
                loss_gen_all=torch.mean(total_loss_reduced)
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                # logger.info([total_loss] + [global_step, lr])
                ##July 22
                logger.info(f'total_loss in syn = {total_loss} + at global step {global_step}, leanring rate is {lr}')
                ##July 22
                log_tensor_values(reward.tolist(), 'reward',logger,global_step, lr)
