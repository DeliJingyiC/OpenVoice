...
real_audio_file='/home/ubuntu/OpenVoice/emotion_STS/melo_rlhf/data/example/esd_info.csv'
...
real_audio_data=shuffle(real_audio_data,n_samples=7061).values.tolist()

# conv_csv_data.remove(['path', 'speaker', 'text', 'text_id', 'emo'])
# real_emo=['ANG', 'DIS','FEA','HAP','NEU','SAD']
# print('real_audio_data',real_audio_data)
# input()
emo_dic = {
    'emo': [row[4] for row in real_audio_data],
    'wav': [row[5] for row in real_audio_data]
}
...
def get_non_matching_paths(emo_dic):
    while True:
        real_path = random.choice(emo_dic['wav'])
        real_emo_path = random.choice(emo_dic['emo'])
        r_emo = real_path.split('/')[-1].split('_')[0]
        r_emo_p = real_emo_path.split('/')[-1].split('_')[0]
        if r_emo != r_emo_p:
...
def get_matching_paths(emo_dic):
    while True:
        ...
        r_speak = real_path.split('/')[-1].split('_')[0]
        r_speak_p = real_emo_path.split('/')[-1].split('_')[0]
        r_word=real_path.split('/')[-1].split('_')[-1].split('.')[0]
        r_word_p=real_emo_path.split('/')[-1].split('_')[-1].split('.')[0]
        print('r_emo',r_emo)
        print('r_emo_p',r_emo_p)
        print('r_word',r_word)
        print('r_word_p',r_word_p)
        print('r_speak',r_speak)
        print('r_speak_p',r_speak_p)

        if r_emo != r_emo_p and r_word==r_word_p and r_speak==r_speak_p:
            print('real_path',real_path)
            print('real_emo_path',real_emo_path)
