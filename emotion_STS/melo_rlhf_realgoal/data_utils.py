...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        for item in tqdm(
            ...
            try:
                goal_syn_audio, spk, input_audio,emo_label, emo,real_input,real_emo,real_goal = item
            except:
                raise
            ...
            realaudio_ref=f'{real_emo}'
            real_goal_au=f'{real_goal}'

            audiopaths_sid_text_new.append(
                    [audiopath, audiopath_input,emopath,realaudio_input,realaudio_ref,real_goal]
                )
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        audiopath, input_audio,emo_audio,real_input,real_ref,real_goal = audiopath_sid_text
        ...
        spec_real_goal,wav_real_goal=self.get_audio(real_goal)
        ...
        return (spec, wav, spec_input,wav_input,emo,spec_input_real,wav_input_real,emo_real,spec_real_goal,wav_real_goal)
...
class TextAudioSpeakerCollate:
    ...
    def __call__(self, batch):
        ...
        max_spec_realgoal_len = max([len(x[8]) for x in batch])
        max_wav_realgoal_len = max([x[9].size(1) for x in batch])
        ...
        spec_realgoal_lengths = torch.LongTensor(len(batch))
        wav_realgoal_lengths = torch.LongTensor(len(batch))
        ...
        spec_realinput_padded = torch.FloatTensor(len(batch), batch[0][5].size(0), max_spec_realgoal_len)
        wav_realinput_padded = torch.FloatTensor(len(batch), 1, max_wav_realinput_len)
        spec_realgoal_padded = torch.FloatTensor(len(batch), batch[0][8].size(0), max_spec_realgoal_len)
        wav_realgoal_padded = torch.FloatTensor(len(batch), 1, max_wav_realinput_len)
        ...
        spec_realgoal_padded.zero_()
        wav_realgoal_padded.zero_()
        realemo.zero_()
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            spec_goal = row[8]
            spec_realgoal_padded[i, :, : spec_goal.size(1)] = spec_goal
            spec_realgoal_lengths[i] = spec_goal.size(1)

            wav_goal = row[9]
            wav_realgoal_padded[i, :, : wav_goal.size(1)] = wav_goal
            wav_realgoal_lengths[i] = wav_goal.size(1)
            # input()
...
class TextAudioSpeakerLoader_RL(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        for item in tqdm(
            ...
            try:
                goal_syn_audio, spk, input_syn_audio,emo, emo_syn,real_input_audio,emo_real,real_goal = item
            ...
            audiopaths_sid_text_new.append(
                    [audiopath, audiopath_input,emopath,audiopath_input_real,emopath_real,real_goal]
                )
            lengths.append(os.path.getsize(audiopath) // (2 * self.hop_length))
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        audiopath_syngoal, input_audio_syn,emo_audio_syn,real_input,real_ref,real_goal = audiopath_sid_text
        #goal syn audio
...
class TextAudioSpeakerCollate_RL:
    ...
    def __call__(self, batch):
        ...
        max_spec_realgoal_len = max([len(x[17]) for x in batch])
        max_wav_realgoal_len = max([x[18].size(1) for x in batch])
        ...
        spec_realgoal_padded = torch.FloatTensor(len(batch), batch[0][17].size(0), max_spec_realgoal_len)
        wav_realgoal_padded = torch.FloatTensor(len(batch), 1, max_wav_realgoal_len)
